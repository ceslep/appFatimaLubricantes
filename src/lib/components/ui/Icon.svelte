<script lang="ts">
  let { name, class: cls = 'w-5 h-5', strokeWidth = 1.8 }: {
    name: string;
    class?: string;
    strokeWidth?: number;
  } = $props();

  // Iconos lucide-style (stroke), sin dependencias externas.
  const icons: Record<string, string> = {
    ...
    'circle-check': '<circle cx="12" cy="12" r="10"/><path d="m8.5 12.5 2.5 2.5 5-5"/>',
